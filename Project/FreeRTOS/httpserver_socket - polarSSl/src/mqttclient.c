...
MQTT_START: 
	  ...
	  if(sessionPresent < 0)
		...
		{
				//订阅消息
				if(MQTTSubscribe(mysock,(s8*)"wtopic",QOS2) < 0)	//mymqttsubtest
				{
						 //重连服务器
						 printf("MQTT>>客户端订阅消息失败...\r\n");
						 //goto MQTT_reconnect;	 //wxf 20180604 comment
				}	
	  }
		...
		while(1)
		{
				...
				if(FD_ISSET(mysock,&readfd) != 0)
				{
						...
						printf("Start Process Data Packet!\r\n");
						if(type != -1)
						{
				...
				if((xTaskGetTickCount() - pubtick) >(5000))
				{
					...
					//发布消息 原topic:DongLuTest
					  ret = MQTTMsgPublish(mysock,(s8*)"mtopic",0, 0,(u8*)"This is my first MQTT programme!",strlen("This is my first MQTT programme!"));
					  if(ret >= 0)
						{
								//表明有数据交换
								no_mqtt_msg_exchange = 0;
...
s32 MQTTClientInit(s32 sock)
{
	  ...
	  //MQTT版本
		...
		//用户名
		connectData.username.cstring = "chisj";	//NULL
		//用户密码
		connectData.password.cstring = "chisj";	//NULL
		//清除会话
